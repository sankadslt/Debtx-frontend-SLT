...
import {List_all_transaction_seq_of_batch_id, Case_Distribution_Details_With_Drc_Rtom_ByBatchId} from "/src/services/case/CaseServices.js";
...
export default function AmendAssignedDRC() {
    ...
    const handledeleteclick =  (RTOM, DRC1, DRC2) => {
        setCpeData(cpeData.filter((item) => !(item.RTOM === RTOM && item.DRC1 === DRC1 && item.DRC2 === DRC2))); // Remove selected entry
    };
        ...
        <div className={GlobalStyle.fontPoppins}>
            ...
            <div className="flex flex-col items-center justify-center mb-4">
                    <div className={`${GlobalStyle.cardContainer}`}>
                        ...
                        <p className="mb-2"><strong>Create DTM: </strong> {transaction[0]?.created_dtm  ? new Date(transaction[0].created_dtm).toISOString().split("T")[0] : "N/A"}</p>
            ...
            <div className={`${GlobalStyle.miniCaseCountBar} `}>
                <div className="flex px-3 py-2 items-center  gap-10 ">
                    ...
                    <div className="flex gap-10">
                        ...
                        value={newEntry.DRC1}
                        onChange={(e) => setNewEntry({ ...newEntry, DRC1: e.target.value })}
                    ...
                    <div className="flex gap-10">
                        <select className={GlobalStyle.selectBox}
                        value={newEntry.RTOM}
                        onChange={(e) => setNewEntry({ ...newEntry, RTOM: e.target.value })}
                        >
                            <option value="" hidden>
                                RTOM
                            </option>
                    ...
                    <div className="flex gap-7">
                        <h1 className={GlobalStyle.headingMedium}>Assigned case count : 0</h1>
                            <input  type="number"
                ...
                <div className="flex px-3 py-2 items-center  gap-10 ">
                    ...
                    <div className="flex gap-4">
                        <select className={GlobalStyle.selectBox}
                        value={newEntry.DRC2}
                        onChange={(e) => setNewEntry({ ...newEntry, DRC2: e.target.value })}
                        >
                            <option value="" hidden>
                                DRC
                            </option>
                        ...
                        <tbody>
                            {filteredData.length > 0 ? (
                                filteredData.map((item, index) => (  
                                    <tr key={item.id} className={index % 2 === 0 ? GlobalStyle.tableRowEven : GlobalStyle.tableRowOdd}>
                                        ...
                                        <td className={GlobalStyle.tableData}>
                                            <button onClick={() => handledeleteclick(item.RTOM, item.DRC1, item.DRC2)} className={`${GlobalStyle.buttonPrimary}`}>
                                                Delete
                                            </button>
                                        </td>
                                    </tr>
                                ...
                                <tr>
                                    <td colSpan="6" className={GlobalStyle.tableData}>No data available.</td> 
                                </tr>
                            )}
                        </tbody>

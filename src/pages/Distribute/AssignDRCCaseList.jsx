...
//   //const drc_id = 7; // Hardcoded DRC ID for testing, replace with dynamic value if needed

//   // Get the drc_id from the URL state or fallback to 7 if not available (for testing)
//   //const drc_id = location?.state?.drc_id ?? 7; // Use 7 only if undefined
...
//         await handleApicall(startDate, endDate);
//       } else {
//         // Optionally clear the end date if the user cancels
//         setEndDate(null);
//       }
//     });
//   }
...
//       // Ensure drc_id is defined
//       if (!drc_id) {
//         throw new Error("DRC ID is not defined.");
//       }
...
//       // Add filter values if applicable
//       if (filterType === "Account No" && filterValue.trim() !== "") {
//         payload.account_no = String(filterValue.trim());
//       } else if (filterType === "Case ID" && filterValue.trim() !== "") {
//         payload.case_id = filterValue.trim();
...
//       // Make the API call (replace '/your-api-endpoint' with the actual endpoint)
...
//     } catch (error) {
//       // Handle errors
//       console.error("Error in handleApicall:", error.message);
//       Swal.fire({
//         title: "Error",
//         text: error.message,
//         icon: "error",
...
export default function AssignDRCsLOG() {
  ...
  const handleFilter = () => {
    ...
    payload.drc_id = drc_id;
    if (startDate) {
      payload.from_date = startDate; // Forslimat: YYYY-MM-DD
    }
    if (endDate) {
      payload.to_date = endDate; // Format: YYYY-MM-DD
    }

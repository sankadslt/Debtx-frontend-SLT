...
const DRCList = () => {
    ...
    const fetchData = async (filters) => {
        ...
        try {
            ...
            setIsFiltered(false);
            Swal.fire("Error", error.message || "No DRC is matching the criteria.", "error");
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleFilter = async () => {
        try {
            ...
            Swal.fire("Error", error.message || "No DRS is matching the criteria", "error");
        }
    };
    ...
    const filteredData = data.filter((row) =>
        ...
        String(row.BusinessRegNo).toLowerCase().includes(searchQuery.toLowerCase()) ||
        ...
        String(row.ServiceCount).toLowerCase().includes(searchQuery.toLowerCase()) ||
    ...
    return (
        <div className={GlobalStyle.fontPoppins}>
            ...
            <div className="flex justify-end mt-8 mb-8 ">
                <div className={`${GlobalStyle.cardContainer} w-auto h-auto py-4 px-4`}>
                            ...
                            paginatedData.map((log, index) => (
                                ...
                                >
                                    ...
                                    <td className={GlobalStyle.tableData}>
                                        <Link to={`/pages/DRC/DRCDetails?drcid=${log.DRCID}`}>{log.ServiceCount}</Link>
                                    </td>
                                    <td className={GlobalStyle.tableData}>
                                        <Link to={`/pages/DRC/DRCDetails?drcid=${log.DRCID}`}>{log.ROCount}</Link>
                                    </td>
                                    <td className={GlobalStyle.tableData}>

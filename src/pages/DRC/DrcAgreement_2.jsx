...
// import { DRC_Agreement_details_list } from "../../services/DRC/Drc";
...
const DrcAgreement = () => {
  ...
  const drc_id = 5; // Assuming a fixed DRC ID for this example
  ...
  const FetchDRCAgreementDetails = async (filters) => {
    try {
      const response = await DRC_Agreement_details_list(drc_id);
      console.log("DRC Agreement Details Response:", response);
      ...
      if (response && response.data) {
        ...
        if (response.data.length === 0) {
          Swal.fire({
            title: "Error",
            text: "Error fetching Agreement Details.",
            icon: "error",
            confirmButtonColor: "#d33"
          })
        } else {
          setAgreementHistory(response.data);
        }
      ...
      Swal.fire({
        title: "Error",
        text: error.message || "Error fetching Agreement Details.",
        icon: "error",
        confirmButtonColor: "#d33"
      });
    } 
  ...
  console.log("Agreement History:", agreementHistory);
                    ...
                    agreementHistory.map((agreement, index) => (
                      ...
                      >
                        <td className={`${GlobalStyle.tableData} text-left text-xs lg:text-sm`}>
                          {formatDateForDisplay(agreement.agreement_start_dtm)}
                        </td>
                        <td className={`${GlobalStyle.tableData} text-left text-xs lg:text-sm`}>
                          {formatDateForDisplay(agreement.agreement_end_dtm)}
                        </td>
                        <td className={`${GlobalStyle.tableData} text-left text-xs lg:text-sm`}>
                          {agreement.agreement_remark}
                        </td>
                        <td className={`${GlobalStyle.tableData} text-left text-xs lg:text-sm`}>
                          {agreement.agreement_update_by}
                        </td>
                      </tr>
                    ))
                  ) : (

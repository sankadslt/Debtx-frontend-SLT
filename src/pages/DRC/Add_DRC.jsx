...
import { getUserDetails } from "../../services/drc/Drc.js";
import {
...
const Add_DRC = () => {
  ...
  // SLT Coordinator details - Updated
  ...
  const fetchSLTCoordinators = async () => {
    try {
      ...
      const response = await getUserDetails(); // Assuming this returns all coordinators when called without ID
      console.log("Coordinators API Response:", response);

      if (response && Array.isArray(response.data || response)) {
        const coordinatorsData = response.data || response;
        const formattedCoordinators = coordinatorsData.map((coord) => ({
        ...
        console.error("Unexpected coordinators API response format:", response);
        setCoordinators([]);
      }
    } catch (error) {
      console.error("Error loading SLT coordinators:", error);
      Swal.fire({
        icon: "error",
        title: "Error",
  ...
  useEffect(() => {
    fetchActiveServices();
    fetchRTOMData();
    fetchSLTCoordinators(); // Added this
  }, []);
  ...
  const validateForm = () => {
    ...
    if (!selectedCoordinator) newErrors.selectedCoordinator = "SLT Coordinator is required.";
          ...
          <tbody>
            <tr className="block md:table-row">
              <td className="block md:table-cell w-full md:w-1/3 text-left pb-1 md:pb-2 whitespace-nowrap">
                <span className="inline-block min-w-[120px] pl-6">Name <span className="text-red-500">*</span></span>
                <span className="ml-8">:</span>
              </td>
              <td className="block md:table-cell w-full md:w-1/3 pb-3 md:pb-2">
                <select
                  value={selectedCoordinator}
                  onChange={handleCoordinatorChange}
                  className={`${GlobalStyle.inputText} w-full`}
                >
                  <option value="">Select SLT Coordinator</option>
                  {coordinatorLoading ? (
                    <option disabled>Loading coordinators...</option>
                ...
                {coordinatorLoading && (
                  <p className="text-gray-500 mt-1">Loading coordinators...</p>
                )}
                {errors.selectedCoordinator && (
                  <p className="text-red-500">{errors.selectedCoordinator}</p>
                )}
              </td>
            </tr>

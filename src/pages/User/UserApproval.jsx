        ...
        if (response.data.length === 0) {
          ...
          const updatedData = await Promise.all(
            response.data.map(async (approval) => {
              ...
              // Try to get user details using user_approver_id
              if (approval.user_approver_id) {
                try {
                  const userDetails = await getUserDetailsById(approval.user_approver_id);
                  if (userDetails.status === "success" && userDetails.data) {
                    userName = userDetails.data.username || "N/A";
                    userEmail = userDetails.data.email || "N/A";
                  }
                } catch (userError) {

...
const RtomInfoNew = () => {
        ...
        <div className="flex justify-center">
            <div className={`${GlobalStyle.cardContainer} p-4`}>
                <div className="flex mb-4 justify-end">
                    {/* Hide edit button if RTOM is terminated */}
                    {!isRtomTerminated() && (
                        <button onClick={switchToEditMode}>
                            <img src={edit_info} title="Edit" className="w-6 h-6" />
                        </button>
                    )}
                    {/* Show terminated status if RTOM is terminated */}
                    {isRtomTerminated() && (
                        <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-2 rounded">
                            <strong>TERMINATED</strong> - This RTOM cannot be edited
                        </div>
                    )}
                </div>
    ...
    const renderCommonUI = () => (
        <div className="flex flex-col">
            {/* Only show End button if RTOM is not terminated */}
            {isEndButtonVisible && mode === 'view' && !isRtomTerminated() && (

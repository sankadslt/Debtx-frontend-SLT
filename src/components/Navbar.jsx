...
const Navbar = () => {
  ...
  const handleTaskClick = (taskId, url) => {
    ...
    if (!task) return;
    if (!task.completed) markTaskAsDoneAndNavigate(taskId, url);
    else navigate(url);
  };
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      if (!userData) return;
      const staticTasks = [
        "Create LOD", "Create FTL LOD", "Create Final Reminder", "MB Sending Request",
        "Agent Time Extend", "Ledger Details", "Nego 1", "Pending Dispute Settlement Letter",
        "Pending Dispute Letter", "Customer Settele the Areas", "Monitoring Completed, Customer NOT Settele the Areas",
        "Monitor Payment of Case", "Case Status File Created", "Validate Incident",
        "Pending Approval Agent Distribution", "Pending Approval RO Extend Period",
        "File Ready to Download", "Agent Distribution Batch Approved",
        "Approved DRC Assign Manager Approval", "Rejected DRC Assign Manager Approval",
        "Pending Mediation Board Handed Over Non-Settlement", "Pending Negotiation Settlement Plan Request",
        "Pending Mediation Board Period Extend Request", "Pending Negotiation Customer Further Information Request",
        "Pending Negotiation Customer Request Service", "Pending Mediation Board Customer Further Information Request",
        "Pending Mediation Board Settlement Plan Request", "Pending Mediation Board Customer Request Service",
        "Pending Approval for Case Withdraw", "Handed Over Non-Settlement",
        "Pending Approval Write-off Case", "Pending Approval for DRC Re Assign Approval"
      ].map((text, idx) => ({
        _id: `${idx + 1}`,
        task: text,
        completed: false,
          ...
          {isTaskListOpen && (
            ...
            >
              ...
              <ul
                ...
                onScroll={(e) => {
                  const { scrollTop, scrollHeight, clientHeight } = e.target;
                  if (scrollTop + clientHeight >= scrollHeight) loadMoreTasks();
                }}
              >
                {taskData
                  .filter((task) => task.user_id === userData?.id)

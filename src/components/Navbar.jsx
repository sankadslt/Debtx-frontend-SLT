...
// // export default Navbar;
 ...
 // Added enhancement: process name rendered in task list items
 ...
 const Navbar = () => {
   ...
   <div
     ref={taskListRef}
     className="absolute top-12 right-0 w-[450px]  bg-[#E1E4F5] bg-opacity-20 backdrop-blur-sm   text-[#00256A] rounded-xl shadow-2xl p-5"
   >
 ...
 hover:border-blue-100 hover:shadow-md"
  ...
  {task.task}
               ...
               {task.showParameters &&
                Array.isArray(task.showParameters) &&
                 task.showParameters.map((paramKey) => (
                  <span key={paramKey}className="text-sm text-gray-900  group-hover:text-white ">
                    {paramKey.replace(/_/g, ' ')}: {task[paramKey]}
                  </span>
                ))
                }
             </div>
             ...
             <button
               onClick={() => handleTaskClick(task._id, task.url)}
               className={`px-3 py-1.5 text-xs font-semibold rounded-full transition-all duration-200 flex-shrink-0 ${
                 task.completed
